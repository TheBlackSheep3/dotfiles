...
def svg-to-png [
    ...
    if (null == $height) {
        `c:\Program Files\Inkscape\bin\inkscape.exe` $svg_file $"--export-filename=($svg_file | path parse | get stem)_($width)x($width).png" $"--export-width=($width)" $"--export-height=($width)"
    } else {
...
source ~/.dotfiles/nushell/bitbucket.nu
